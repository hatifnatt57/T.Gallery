...
function generateElement($link) {
  $caption = $link['text'];
  if ($link['author'] != "")
    $caption = $link['author']." &middot; ".$caption;
return <<<ELEM
  <li data-id="{$link['id']}">
    <div class="caption">
      <p>{$caption}</p>
...
<head>
  ...
  <title>Редактирование раздела НамПишут | T.Gallery</title>
</head>
<body>
  <?php
  ...
  ?>
  <header>
    <a href="../" class="back-link" title="На главную страницу">
    ...
    <a href="./add/" class="add-link" title="Добавить запись">
  ...
  <?php
    if (count($allLinks) === 0) {
      ...
      echo '<ul class="ul-links">';
      foreach ($allLinks as $link) { echo(generateElement($link)); };
      echo '</ul>';
    }
  ?>
  <script>
